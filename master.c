...
static int adder_loop(const unsigned int N){

	int index = 0;
	unsigned int len = 2;  //each process gets a pair of ints
...
static int group_results(const unsigned int len){
	int i, j=1;
	for(i=2; i < len; i+=2){
		ossaddr->numbers[j++] = ossaddr->numbers[i];
	}
	return len / 2;	//the new N
}
